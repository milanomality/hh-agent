...
запросам, оценивает каждую против резюме (Claude), пишет сопроводительные письма
и присылает карточки в Telegram. Ведёт локальную воронку откликов.
...
```
hh.ru API ──► Поллер (APScheduler, каждые N минут)
                 │  дедупликация по SQLite («уже видел»)
                 ▼  новые вакансии
            Скоринг (Claude, structured outputs → ScoreResult)
                 │  score >= порога → сопроводительное письмо
                 ▼
            Telegram-карточка: оценка, разбор, письмо
            [🔗 Открыть на hh.ru]  ← URL-кнопка
            [✅ Откликнулся] [⏭ Пропустить] [⭐ В избранное]
...
```
hh-agent/
├── PLAN.md, README.md, pyproject.toml, .env.example, .gitignore   [оркестратор]
├── src/hh_agent/
│   ├── __init__.py, config.py, models.py, interfaces.py           [оркестратор — КОНТРАКТЫ]
│   ├── hh/         client.py                                      [модуль hh-api]
│   ├── ai/         scorer.py, prompts.py                          [модуль ai-core]
│   ├── bot/        bot.py, handlers.py, cards.py, notifier.py     [модуль telegram-bot]
│   ├── db/         storage.py                                     [модуль core]
│   ├── scheduler.py                                               [модуль core]
│   └── main.py     (composition root)                             [модуль core]
└── tests/          test_hh_client / test_bot_cards / test_scoring / test_storage / test_scheduler
```
...
| скоринг/письма | `from hh_agent.ai.scorer import ClaudeScorer` | `ClaudeScorer(settings)` |
| Telegram-бот | `from hh_agent.bot.bot import create_bot` | `create_bot(BotDeps(...)) -> (Bot, Dispatcher, NotifierProto)` |
...
`BotDeps` — storage, scorer, settings (hh боту больше не нужен).
Callback-данные кнопок: `applied:{vacancy_id}`, `skip:{vacancy_id}`, `fav:{vacancy_id}`;
отклик — URL-кнопка на `vacancy.url`.
...
Python 3.12 (async), httpx, aiogram ≥3.29, aiosqlite, APScheduler,
